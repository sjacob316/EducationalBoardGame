...
namespace ViscardiDigitalBoardGame
{
    public partial class playgame : System.Web.UI.Page
    {
        ...
        protected void ans4_Click1(object sender, EventArgs e)
        {
            ...
            while (readerJ.Read())
            {
                if (ans4.Text == (String.Format("{0}", readerJ[0])))
                {
                    //ytemp = 185;
                    //xtemp = 675;
                    Session["thepos"] = ytemp;
        ...
        protected void ans2_Click(object sender, EventArgs e)
        {
            SqlCommand scmdans = new SqlCommand("select answer from mathquestions where questionid = '" + i + "'", connection);
            connection.Open();
            SqlDataReader readerJ = scmdans.ExecuteReader();
            while (readerJ.Read())
            {
                if (ans2.Text == (String.Format("{0}", readerJ[0])))
                {
                    Session["thepos"] = ytemp;
                    Session["opos"] = xtemp;
                    position = moves + position;
